...
def record_unanswered(text: str, user_id: Optional[str] = None, tenant_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    p = config.get_support_db_path(tenant_id)
    db_path = p
    ts = int(time.time())
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def load_course_catalog(tenant_id: Optional[str] = None) -> Dict[str, Any]:
    base = config.get_kb_data_dir(tenant_id)
        ...
        with open(p, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if t.startswith("###"):
                    ...
                    sections.append(sec)
                if ("Q：" in t) or ("Q:" in t):
                    q = t.split("Q：", 1)[-1] if "Q：" in t else t.split("Q:", 1)[-1]
                    q = q.strip()
                    items.append({"section": sec, "q": q})
                if ("A：" in t) or ("A:" in t):
                    a = t.split("A：", 1)[-1] if "A：" in t else t.split("A:", 1)[-1]
                    a = a.strip()
                    if items:
                        items[-1]["a"] = a
    except Exception:
        return {"sections": [], "items": []}
    return {"sections": sections, "items": items[:20]}
