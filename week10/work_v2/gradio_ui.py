...
def _load_orders() -> List[List[Any]]:
    p = config.get_orders_db_path()
    if not p:
        return []
    try:
...
def build_orders_ui():
    ...
    with gr.Blocks() as demo:
        gr.Markdown("订单数据库")
        df = gr.Dataframe(headers=headers, value=_load_orders(), interactive=False)
        btn = gr.Button("刷新")
        btn.click(fn=lambda: _load_orders(), inputs=None, outputs=df, api_name="lambda", show_progress="minimal")
    return demo
