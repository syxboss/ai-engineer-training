...
/**
 * 处理事件接口 - 用于表示时间轴中的单个事件
 * ProcessedEvent interface - represents a single event in the timeline
 */
export interface ProcessedEvent {
  title: string; // 事件标题
  data: any;     // 事件数据（可以是字符串、数组或对象）
}

/**
 * 活动时间轴组件属性接口 - 定义组件接收的属性
 * ActivityTimeline component props interface - defines the props received by the component
 */
interface ActivityTimelineProps {
  processedEvents: ProcessedEvent[]; // 处理过的事件数组
  isLoading: boolean;                // 是否正在加载状态
}

/**
 * 活动时间轴组件 - 显示研究过程的时间轴视图
 * ActivityTimeline component - displays a timeline view of the research process
 * 
 * @param processedEvents - 处理过的事件数组
